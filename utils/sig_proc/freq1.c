...
double
freq1(int n, double data[], int SMOOTH_FLAG)
{
	int w = min(n/500, 30);  //width of smoothing kernel - but not too big
	double f[n], A[n];
	freqspec(n, data, f, A);  //Get the frequency spectrum
	if (SMOOTH_FLAG) { gsmoothip(n, A, w); }  //smooth fft -> reliable peaks
	return f[maxindex(n, A)];  //return the frequency of largest amp peak
}
